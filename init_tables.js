...
client.query('CREATE TABLE `statistic`.`pv_day` (`date` DATE NOT NULL, `today` BIGINT, `total` BIGINT, CONSTRAINT `PK_pv_day` PRIMARY KEY (`date`)) ENGINE=InnoDB DEFAULT CHARSET=utf8 ', {}, function (err, result) {
...
client.query('CREATE TABLE `statistic`.`pv_visitor` (`uid` VARCHAR(50) NOT NULL, `host` VARCHAR(50), `date_created` DATETIME,          `user_agent` VARCHAR(200), `last_visit_time` DATETIME, `active` INT, CONSTRAINT `PK_pv_visitor` PRIMARY KEY (`uid`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8 ', {}, function (err, result) {
